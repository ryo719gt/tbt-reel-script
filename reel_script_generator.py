...
def save_telop_script(content: str, name: str) -> str:
    """テロップリールを output/telop_reels/{YYYYMMDD_HHMM}_{name}.md に保存する。"""
    output_dir = os.path.join(BASE_DIR, "output", "telop_reels")
    os.makedirs(output_dir, exist_ok=True)
    safe_name = name.replace(" ", "_").replace("/", "_")[:40]
    output_path = os.path.join(output_dir, f"{_timestamp()}_{safe_name}.md")
    with open(output_path, "w", encoding="utf-8") as f:
        f.write(content)
    subprocess.run(["open", "-a", "Obsidian", output_path])
    return output_path
...
def _run_talk_reel_flow(topic: str) -> tuple[str, str]:
    """トークリール生成→評価→改善ループを実行し、(最終台本, 保存フォルダ) を返す"""
    print(f"\nトークリール台本を生成中...（テーマ：{topic}）\n")
    script = generate_talk_reel_script(topic)
    print(script)
    # 最初の保存時にフォルダを確定させ、以降はそこに追記
    ...
    if score < 12.0:
        ...
        if ans == "y":
            ...
            print(improved)
            path = save_talk_content(improved, topic, "台本_改善版.md", folder=folder)
            print(f"\n保存先: {path}")
...
def main():
    ...
    if "--talk" in sys.argv:
        ...
        _offer_stories_threads(final_script, topic, folder=folder)
        ...
        if mode == "7":
            ...
            if score < 12.0:
                ...
                if ans == "y":
                    ...
                    print(f"\n保存先: {path}")
                    _offer_stories_threads(improved, topic, folder=os.path.dirname(path))
            else:
                print(f"\n合計 {score}/15 — 品質基準クリア。")
